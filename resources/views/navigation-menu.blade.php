<nav class="mt-5 px-2 h-[calc(100vh-156px)] flex flex-col justify-between">
    <div class="space-y-1">
        ...
        <a href="{{ route('groups') }}"
           class="{{ request()->routeIs('groups*') ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }} group flex items-center px-2 py-2 text-sm font-medium rounded-md">
            <x-heroicon-o-user-group
                    class="{{ request()->routeIs('groups*') ? 'text-gray-300' : 'text-gray-400 group-hover:text-gray-300' }} mr-3 flex-shrink-0 h-6 w-6"></x-heroicon-o-user-group>
            Gruppi di appartenenza
        </a>
        <a href="{{ route('users') }}"
           class="{{ request()->routeIs('users*') ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }} group flex items-center px-2 py-2 text-sm font-medium rounded-md">
            <x-heroicon-o-users
                    class="{{ request()->routeIs('users*') ? 'text-gray-300' : 'text-gray-400 group-hover:text-gray-300' }} mr-3 flex-shrink-0 h-6 w-6"></x-heroicon-o-users>
            Utenti
        </a>
    </div>
</nav>
