...
<div>
	...
	@endif
	<div class="mt-1">
		<textarea {{ $disabled ? 'disabled' : '' }} name="{{ $for }}" id="{{ $for }}" {!! $attributes->merge(['class' => 'focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md']) !!}>
			{{ $slot }}
		</textarea>
	</div>
